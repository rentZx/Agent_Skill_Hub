...
async function searchRepositories(query: string): Promise<GitHubSearchItem[]> {
  ...
  const response = await fetch(`https://api.github.com/search/repositories?q=${encodeURIComponent(query)}&sort=stars&order=desc&per_page=8`, { headers, cache: "no-store" });
  if (!response.ok) {
