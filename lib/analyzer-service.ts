...
export async function analyzeProjectWithAI(input: string, resources: Resource[]): Promise<AnalyzerResult & { source: "deepseek" | "rules"; discoveredCount: number }> {
  ...
  try {
    ...
    console.warn(`GitHub discovery runtime: token=${Boolean(process.env.GITHUB_TOKEN)} tokenLength=${process.env.GITHUB_TOKEN?.length ?? 0} candidates=${discovered.length}`);
  } catch (error) {
